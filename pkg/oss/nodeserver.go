...
func (ns *nodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if err := checkOssOptions(opt); err != nil {
		log.Errorf(log.TypeOSS, log.StatusArgsInvalid, "NodePublishVolume:: Check oss input error: %s", err.Error())
		return nil, errors.New("Check oss input error: " + err.Error())
	}
	if mountPath == "" {
		log.Errorf(log.TypeOSS, log.StatusArgsInvalid, "NodePublishVolume:: Check oss input error: mountPath is empty ")
		return nil, errors.New("mountPath is empty")
	}

	log.Infof(log.TypeOSS, log.StatusOK, "NodePublishVolume:: Starting Oss Mount: Bucket: %s, url: %s, OtherOpts: %s, Path: %s, UseSharedPath: %s, autType: %s", opt.Bucket, opt.URL, opt.OtherOpts, opt.Path, strconv.FormatBool(opt.UseSharedPath), opt.AuthType)
	...
	if opt.AuthType != "sts" {
		...
		if err := saveOssCredential(opt); err != nil {
			log.Errorf(log.TypeOSS, log.StatusInternalError, "NodePublishVolume:: Oss, Save AK file fail: %s", err.Error())
	...
	if opt.UseSharedPath {
		...
		if IsOssfsMounted(sharedPath) {
			...
			if err := utils.CreateDest(sharedPath); err != nil {
				log.Errorf(log.TypeOSS, log.StatusExecuCmdFailed, "NodePublishVolume:: Create Directory error: %s", err.Error())
			...
			if out, err := connectorRun(mntCmd); err != nil {
				if err != nil {
					log.Errorf(log.TypeOSS, log.StatusMountFailed, "NodePublishVolume:: Ossfs mount error: %s", err.Error())
		...
		if err := ns.k8smounter.Mount(sharedPath, mountPath, "", options); err != nil {
			log.Errorf(log.TypeOSS, log.StatusMountFailed, "NodePublishVolume:: Ossfs mount error: %v", err.Error())
			return nil, errors.New("Create OSS volume fail: " + err.Error())
		}
	} else {
		// Create Mount Path
...
func saveOssCredential(options *Options) error {
	...
	if err := ioutil.WriteFile(CredentialFile, []byte(newContentStr), 0640); err != nil {
		log.Errorf(log.TypeOSS, log.StatusInternalError, "NodePublishVolume:: Write Credential failed err, %s, %s", newContentStr, err)
		return err
	}
	return nil
}
...
func connectorRun(cmd string) (string, error) {
	...
	if err != nil {
		log.Errorf(log.TypeOSS, log.StatusInternalError, "NodePublishVolume:: Oss connector Dial error: %s", err.Error())
	...
	if err != nil {
		log.Errorf(log.TypeOSS, log.StatusInternalError, "NodePublishVolume:: Oss connector write error: %s", err.Error())
		return err.Error(), err
	}
...
func (ns *nodeServer) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	if !IsOssfsMounted(mountPoint) {
		log.Infof(log.TypeOSS, log.StatusOK, "NodeUnpublishVolume:: Directory is not mounted: %s",  mountPoint)
	...
	if IsOssfsMounted(sharedMountPoint) {
		...
		if err != nil {
			log.Errorf(log.TypeOSS, log.StatusExecuCmdFailed, "NodeUnpublishVolume:: Umount oss fail, with: %s",  err.Error())
	...
	log.Infof(log.TypeOSS, log.StatusOK, "NodeUnpublishVolume:: Umount OSS successfully: %s", mountPoint)
	return &csi.NodeUnpublishVolumeResponse{}, nil
}
